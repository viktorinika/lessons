...
function initProducts(data) {
	...
	if (productsList.length) {
		...
		productsList.forEach(productItem => {
			productTemplete += `<article class="item-product">`
			productTemplete += `<a href="#" class="item-product__favorite ${productItem.favorite ? 'item-product__favorite--active' : null} _icon-heart"></a>`;
			if (productItem.image) {
				productTemplete += ` <a href="${productItem.url}" class="item-product__img-link">`;
				productTemplete += `<img class="item-product__img" src="${productItem.image}" alt="image">`;
				productTemplete += ` </a>`
			}
			productTemplete += `<div class="item-product__body">`;
			productTemplete += `<h4 class="item-product__title">`;
			productTemplete += `<a href="${productItem.url}" class="item-product__link-title">${productItem.title}</a>`;
			productTemplete += `</h4>`
			if (productItem.label) {
				productTemplete += `<div class="item-product__label">${productItem.label}</div>`;
			}
			productTemplete += ` <div class="item-product__price">${productItem.price}</div>`;
			productTemplete += `</div>`;
			productTemplete += `</article>`;
		});
		catalogItems.innerHTML = productTemplete;
	}
}
...
function setTab (tabElement) {
	...
	const currentButtonsIndex = tabsButtons.indexOf(tabElement);
	const tabxElements = tabsParent.querySelectorAll('[data-tabs-element]');

	tabxElements.forEach(tabxElement => {
		tabxElement.hidden = true;
	});

	tabxElements[currentButtonsIndex].hidden = false;
}